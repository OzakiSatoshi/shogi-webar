...
### `register.html`（実物の駒を登録してAR・実験 / R3）
- 自分の物理駒の面をカメラで**撮影→ターゲット登録→その場でコンパイル→AR表示**するフロー。
- 撮影した駒ごとに駒種を割り当て、各駒の上にキャラを表示。登録は IndexedDB に保存し再利用。
- 注意：実物の駒は特徴点が乏しく追従は不安定になりやすい（MindARの原理的制約）。明るく・正面・大きく撮るほど安定。
  同種駒（歩9枚など）はほぼ同一のため個体識別は困難。
